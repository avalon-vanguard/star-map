...
describe('GalaxySystemSceneComponent camera-flight transitions', () => {
  ...
  it('keeps the stars of a plotted route drawn, and the selected star', async () => {
    ...
    component.routeResult.set({ stars: [{ id: SUN.id, name: 'Sol' }, { id: PROXIMA.id, name: 'Proxima Centauri' }], totalPc: 1.3, neededRangePc: null, gaveUp: false });
    await advanceFrames(engine, 0.3);
  ...
  describe('the jump-link graph', () => {
    ...
    it('links the stars being drawn, and asks again once a new set of them holds still', async () => {
      ...
      component.routeResult.set({ stars: [{ id: SUN.id, name: 'Sol' }], totalPc: 0, neededRangePc: null, gaveUp: false });
      await advanceFrames(engine, 0.3);
      await settle();
      expect(links).toHaveBeenCalledTimes(2);
    });
  ...
  it('shows the answer to the latest route asked for, whatever order the answers arrive in', async () => {
    type Answer = { route: { stars: number[]; totalPc: number; longestHopPc: number } | null; neededRangePc: number | null; gaveUp: boolean };
    ...
    answers[1]({ route: { stars: [SUN.id, PROXIMA.id], totalPc: 1.3, longestHopPc: 1.3 }, neededRangePc: null, gaveUp: false });
    await flushAsync();
    answers[0]({ route: { stars: [SUN.id, ALPHA_CENTAURI.id], totalPc: 1.34, longestHopPc: 1.34 }, neededRangePc: null, gaveUp: false });
    await flushAsync();
    ...
    answers[2]({ route: null, neededRangePc: null, gaveUp: true });
    await flushAsync();
